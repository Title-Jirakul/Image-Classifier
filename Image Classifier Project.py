...
classifier = nn.Sequential(OrderedDict([
                          ...
                          ('relu3', nn.ReLU()),
                          ('dropout3', nn.Dropout(p_dropout)),
                          ('output', nn.Linear(hidden_layers[2], output_size)),
...
def load_checkpoint(filepath):
    ...
    classifier = Network(checkpoint['input_size'], checkpoint['output_size'], checkpoint['hidden_layers'], checkpoint['dropout'])
...
# Write a function that loads a checkpoint and rebuilds the mode
def load_checkpoint(filepath):
    checkpoint = torch.load(filepath)
    load_model = models.vgg16(pretrained=True)
    load_model.to('cuda')
...
# Printing the original version of the image
imagedatasets = datasets.ImageFolder(train_dir)
im = imagedatasets[0]
im[0]
...
def predict(image_path, model, topk=5):
    ...
    probs = top5[0].cpu().numpy()[0]
    index_classes = top5[1].cpu().numpy()[0]
...
# Create lists of flower names and their corresponding probabilities
...
data = {'Flowers' : flower_names,
       'Probabilities' : probs}

# Create a DataFrame and provide the row index
df = pd.DataFrame(data, index = ['label 1', 'label 2', 'label 3', 'label 4', 'label 5'])
...
image_path = 'flowers/train/1/image_06734.jpg'
flower = imshow(process_image(image_path))
flower.set_title(flower_names[0])
flower.axis('off');
...
ax = sb.barplot(x = 'Probabilities', y = 'Flowers', data=df, color = base_color)
...
img = mpimg.imread('flowers/train/1/image_06735.jpg')
...
ax2.barh(range(len(flower_names)), list(map(float,probs)), tick_label = flower_names)
